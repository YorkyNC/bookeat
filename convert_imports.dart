...
void main(List<String> args) {
  ...
  if (!libDir.existsSync()) {
    log('Error: lib directory not found in $projectRoot');
    exit(1);
  }
  ...
  if (!pubspecFile.existsSync()) {
    log('Error: pubspec.yaml not found in $projectRoot');
    exit(1);
  }

  final packageName = _getPackageName(pubspecFile);
  if (packageName == null) {
    log('Error: Could not find package name in pubspec.yaml');
    exit(1);
  }

  log('Converting imports for package: $packageName');
  log('Processing files in: ${libDir.path}');

  int filesProcessed = 0;
  int importsConverted = 0;

  // Process all Dart files in lib directory
  libDir.listSync(recursive: true).whereType<File>().where((file) => file.path.endsWith('.dart')).forEach((file) {
    final result = _processFile(file, packageName, libDir.path);
    if (result.modified) {
      filesProcessed++;
      importsConverted += result.conversions;
      log('✓ ${path.relative(file.path, from: projectRoot)} - ${result.conversions} imports converted');
    }
  });

  log('\n🎉 Conversion complete!');
  log('Files processed: $filesProcessed');
  log('Total imports converted: $importsConverted');
}
...
ProcessResult _processFile(File file, String packageName, String libPath) {
  ...
  for (final line in lines) {
    ...
    if (trimmed.startsWith('import \'package:$packageName/') || trimmed.startsWith('export \'package:$packageName/')) {
      final isExport = trimmed.startsWith('export');
      final keyword = isExport ? 'export' : 'import';

      // Extract the import path
      final match = RegExp('$keyword\\s+[\'"]package:$packageName/([^\'";]+)[\'"]').firstMatch(trimmed);
      final importedPath = match?.group(1);

      if (importedPath != null) {
        final relativePath = _calculateRelativePath(file.path, importedPath, libPath);

        if (relativePath != null) {
          final newLine = line.replaceFirst('package:$packageName/$importedPath', relativePath);
...
String? _calculateRelativePath(String fromFile, String toPath, String libPath) {
  try {
    ...
    log('Warning: Could not calculate relative path for $toPath from $fromFile');
    return null;
  }
}
